...
**Статус реализации (на 20 мая 2026):** все обязательные пункты раздела 3 курса закрыты в коде и прогнаны. Stack поднимается через `docker compose up`, e2e проверен через Playwright (форма → план → правка → accept → PDF). Evals A/B прогнан на 10 golden-примерах — отчёт в `evals/results/ab_mini_41_vs_4o.md`. Остаётся: презентация.
...
| Mini vs full | mini выбран осознанно — достаточно для генерации плана из готового RAG-контекста, скорость в 2-3× выше | Дедлайн жёсткий, full-модель не нужна |
